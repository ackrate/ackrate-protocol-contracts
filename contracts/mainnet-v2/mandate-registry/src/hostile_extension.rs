...
use crate::{Error, MandateRegistry, MandateRegistryClient};
...
fn hostile_extension_cannot_bypass_revocation_expiry_or_pause() {
    ...
    PrincipalClient::new(&revoked.env, &revoked.user)
        .revoke(&revoked.registry, &revoked.mandate_id);
    assert!(revoked
        .hostile()
    ...
    PrincipalClient::new(&paused.env, &paused.admin).pause(&paused.registry);
    assert!(paused
        .hostile()
...
fn hostile_extension_cannot_select_a_different_merchant_or_asset() {
    let world = World::new();
    let other_asset_admin = world.env.register(Principal, ());
    let other_asset = world
        .env
        .register_stellar_asset_contract_v2(other_asset_admin.clone())
        .address();
    PrincipalClient::new(&world.env, &other_asset_admin).mint(&other_asset, &world.user, &MAX);
